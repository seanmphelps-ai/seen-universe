...
type SpeechRecognitionInstance = { lang: string; interimResults: boolean; continuous: boolean; onresult: ((event: SpeechRecognitionResultEvent) => void) | null; onerror: (() => void) | null; onend: (() => void) | null; start: () => void; stop: () => void };
...
function MicrophoneIcon() { return <svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true" style={{ display: 'block', fill: 'none', stroke: 'currentColor', strokeWidth: 1.8, strokeLinecap: 'round', strokeLinejoin: 'round' }}><rect x="9" y="3" width="6" height="11" rx="3" /><path d="M6 11a6 6 0 0 0 12 0M12 17v4M9 21h6" /></svg>; }
...
export default function ForgeLocationPage() {
  ...
  const [birthLocation, setBirthLocation] = useState(''); const [birthStartYear, setBirthStartYear] = useState(''); const [birthEndYear, setBirthEndYear] = useState(''); const [showBirthPeriod, setShowBirthPeriod] = useState(false);
  ...
  const [error, setError] = useState(''); const [listening, setListening] = useState(false);

  function startDictation() {
    const speechWindow = window as typeof window & { SpeechRecognition?: SpeechRecognitionConstructor; webkitSpeechRecognition?: SpeechRecognitionConstructor };
    const Recognition = speechWindow.SpeechRecognition ?? speechWindow.webkitSpeechRecognition;
    if (!Recognition) { setError('Voice dictation is not available in this browser.'); return; }
    const recognition = new Recognition(); recognition.lang = 'en-US'; recognition.interimResults = false; recognition.continuous = false;
    recognition.onresult = (event) => { const transcript = event.results[0]?.[0]?.transcript?.trim(); if (transcript) sessionStorage.setItem('seen.foundation.locationTranscript', transcript); setListening(false); };
    recognition.onerror = () => { setListening(false); setError('Voice dictation stopped. Tap the microphone to try again.'); };
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalizedBirthLocation || !normalizedCurrentLocation) { setError('Enter your birth location and current location to continue.'); return; }
    setError(''); sessionStorage.setItem('seen.foundation.locations', JSON.stringify({ birthLocation: normalizedBirthLocation, birthPeriod: { startYear: birthStartYear || null, endYear: birthEndYear || null }, livedLocations: normalizedLivedLocations, livedPeriods, currentLocation: normalizedCurrentLocation, currentPeriod: { startYear: currentStartYear || null, endYear: 'present' }, minimumResidenceMonths: 6 })); router.push('/foundation/birth');
  }

  return <main className="seenForgePage"><Image className="seenForgeBackdrop" src="/foundation/location-forge-background.png" alt="" aria-hidden="true" fill priority sizes="(max-width: 760px) 100vw, 760px" /><div className="seenForgeBackdropVeil" aria-hidden="true" /><section className="seenForgeShell" aria-labelledby="forge-title"><header className="seenForgeMasthead"><span className="seenForgeNumber">01</span><h1 id="forge-title" className="seenForgeTitle">The Forge</h1></header><div className="seenForgeGlobeSpace" aria-hidden="true" /><section className="seenForgeExposure" aria-labelledby="exposure-title"><header className="seenForgeExposureHeader"><h2 id="exposure-title">What were you exposed to?</h2></header>
    <button type="button" onClick={startDictation} aria-label="Speak your location history" title="Speak your location history" style={{ display: 'grid', width: 44, height: 44, margin: '0 0 12px auto', padding: 0, placeItems: 'center', color: listening ? '#fff1bd' : '#e7b852', background: 'rgba(6,5,3,.88)', border: '1px solid rgba(246,188,69,.92)', borderRadius: '50%', boxShadow: listening ? '0 0 18px rgba(255,177,39,.9)' : '0 0 8px rgba(255,169,37,.45)', cursor: 'pointer' }}><MicrophoneIcon /></button>
    <form className="seenForgeForm" onSubmit={handleSubmit}>
      <ForgeLocationField label="Birth Location"><LocationAutocompleteInput id="birth-location" className="seenForgeInput" ariaLabel="Birth location" placeholder="Enter city, state, country" value={birthLocation} onChange={setBirthLocation} onLocationEntered={() => setShowBirthPeriod(true)} /></ForgeLocationField>{showBirthPeriod && birthLocation.trim() && <YearPeriodBubble startYear={birthStartYear} endYear={birthEndYear} onStartYearChange={setBirthStartYear} onEndYearChange={setBirthEndYear} />}
      <fieldset className="seenForgeFieldset"><legend className="seenVisuallyHidden">Locations lived six months or longer</legend><div className="seenForgeLocationList">{livedLocations.map((location, index) => <div className="seenForgeLivedRow" key={location.id}><ForgeLocationField label={index === 0 ? 'Locations Lived 6+ Months' : `Lived Location ${index + 1}`}><LocationAutocompleteInput className="seenForgeInput" ariaLabel={`Location lived six months or longer ${index + 1}`} placeholder="Enter city, state, country" value={location.value} onChange={(next) => updateLivedLocation(location.id, next)} onLocationEntered={() => updateLivedPeriod(location.id, 'showPeriod', true)} /></ForgeLocationField>{location.showPeriod && location.value.trim() && <YearPeriodBubble startYear={location.startYear} endYear={location.endYear} onStartYearChange={(value) => updateLivedPeriod(location.id, 'startYear', value)} onEndYearChange={(value) => updateLivedPeriod(location.id, 'endYear', value)} />}{(location.value || livedLocations.length > 1) && <button className="seenForgeRemove" type="button" aria-label={`Remove lived location ${index + 1}`} onClick={() => removeLivedLocation(location.id)}>×</button>}</div>)}</div><button className="seenForgeAdd" type="button" onClick={addLivedLocation}><span aria-hidden="true">+</span>Add another location</button></fieldset>
      <ForgeLocationField label="Current Location"><LocationAutocompleteInput id="current-location" className="seenForgeInput" ariaLabel="Current location" placeholder="Enter city, state, country" value={currentLocation} onChange={setCurrentLocation} onLocationEntered={() => setShowCurrentPeriod(true)} /></ForgeLocationField>{showCurrentPeriod && currentLocation.trim() && <YearPeriodBubble startYear={currentStartYear} present onStartYearChange={setCurrentStartYear} />}
